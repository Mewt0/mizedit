...
briefingRoom.playerPilotNames = { "Weasel 5 1","Weasel 5 2", }
...
function dcsExtensions.vec2ToStringCoordinates(vec2)
  ...
  cooString = cooString.."\nMGRS: "..mgrsString
  cooString = cooString.."\nВысота: "..math.floor(alt * 3.281).."ft"
...
-- * 2 - TOOLS                                                                       *
-- ***********************************************************************************
briefingRoom.playerCoalition = coalition.side.BLUE
briefingRoom.enemyCoalition = coalition.side.RED
-- ===================================================================================
...
briefingRoom.radioManager.enableAudioMessages = true -- Should audio radio messages be played?
...
briefingRoom.eventHandler.BDASetting = "ALL"
...
function briefingRoom.handleGeneralPlayerKill(event)
  ...
  if event.target:getCoalition() ~= briefingRoom.playerCoalition then -- unit is an enemy, radio some variation of a "enemy destroyed" message
    ...
    local messages = { "Оружие было эффективным.", "Хороший удар! Хороший удар!", "Они падают.", "Один разбрызган!" }
    ...
    if briefingRoom.eventHandler.BDASetting == "ALL" then
      briefingRoom.radioManager.play("Команда: "..playerName.." "..messages[messageIndex + messageIndexOffset], "RadioHQ"..soundName..targetType..tostring(messageIndex), math.random(1, 3))
    end
    briefingRoom.aircraftActivator.possibleResponsiveSpawn()
  else
    briefingRoom.radioManager.play("Команда: "..playerName.." Прекратить огонь! Прекратить огонь! Вы стреляете по своим!", "RadioHQTeamkill", math.random(1, 3))
  end 
end
...
function briefingRoom.transportManager.addTroopCargo(transportUnitName, unitNames)
  ...
  for index, unitName in ipairs(unitNames) do
    ...
    if unitCount == maxUnitTroops then
      briefingRoom.radioManager.play("Отряд: Пилот, у нас полно, поехали. (Всего войск: "..maxUnitTroops..")", "RadioTroopFull")
      return true
    end
  ...
  if addedCount > 0 then
    briefingRoom.radioManager.play("Отряд: Пилот, Все собрались, поехали. (Всего войск: "..table.count(briefingRoom.transportManager.transportRoster[transportUnitName].troops)..")", "RadioTroopAllIn")
  end
end
...
function briefingRoom.transportManager.removeTroopCargo(transportUnitName, unitNames, unitPos)
  ...
  if unitPos == nil then
    briefingRoom.radioManager.play("Отряд: Пилот, Все ясно, можно взлетать. (Оставшиеся войска: "..table.count(briefingRoom.transportManager.transportRoster[transportUnitName].troops)..")", "RadioTroopTakeoff")
  end
  return removed
end
...
briefingRoom.mission.autoEnd = false
briefingRoom.mission.commandEnd = false
briefingRoom.mission.MapMarkers = true
briefingRoom.mission.objectiveDropDistanceMeters = 500
...
function briefingRoom.mission.coreFunctions.completeObjective(index, failed)
  ...
  if missionOver then
    ...
    briefingRoom.radioManager.play("Команда: "..(hasFailed and "Миссия не прервана." or "Отличная работа! Миссия завершена, вы можете вернуться на базу."), (hasFailed and  "RadioHQMissionFailed" or "RadioHQMissionComplete"), math.random(6, 8))
    ...
    if briefingRoom.mission.commandEnd then
      missionCommands.addCommandForCoalition(briefingRoom.playerCoalition, "Завершить миссию", nil, briefingRoom.f10MenuCommands.endMission, nil)
    end
  elseif not briefingRoom.mission.hasStarted then
    briefingRoom.radioManager.play("Автозавершенная цель «..objName..» (если вы не используете DSMC и не редактировали миссию в ME, сообщите об ошибке (редактирование целевых юнитов может привести к поломке скриптов))", "Radio0", math.random(6, 8))
  else
    briefingRoom.radioManager.play("Команда: "..(failed and "Задача «..objName..» не выполнена, перейдите к следующей цели." or "Хорошая работа! Задача «..objName..» выполнена, переходим к следующей цели."), (failed and "RadioHQObjectiveFailed" or "RadioHQObjectiveComplete"), math.random(6, 8))
    if revealObjective > 0 then 
      ...
      for j = 1, revealObjective, 1 do
        ...
        briefingRoom.radioManager.play("Команда: Тревога! Новая цель назначена: "..briefingRoom.mission.objectives[index + j].task, "RadioHQNewObjective", time)
      end
    end
  end
end
...
function briefingRoom.f10MenuCommands.missionStatus()
  ...
  if briefingRoom.mission.complete then
    msnStatus = "Команда: Mission complete, you may return to base.\n\n"
    msnSound = "RadioHQMissionStatusComplete"
  else
    msnStatus = "Команда: Mission is still in progress.\n\n"
  ...
  briefingRoom.radioManager.play("Пилот: Команда, требует обновления статуса миссии.", "RadioPilotMissionStatus")
...
function briefingRoom.f10MenuCommands.getWaypointCoordinates(index)
  ...
  briefingRoom.radioManager.play("Пилот: Команда, запрос подтверждения координат путевой точки "..briefingRoom.mission.objectives[index].name.." координаты.", "RadioPilotWaypointCoordinates")
  briefingRoom.radioManager.play("Команда: Подтверждено, передаются координаты путевой точки "..briefingRoom.mission.objectives[index].name.." координаты.\n\n"..cooMessage, "RadioHQWaypointCoordinates", briefingRoom.radioManager.getAnswerDelay())
  ...
  briefingRoom.mission.objectives[index].f10Commands[idx].commandPath = missionCommands.addCommandForCoalition(briefingRoom.playerCoalition, "Координаты маршрутной точки:\n"..cooMessage, briefingRoom.f10Menu.objectives[index], briefingRoom.f10MenuCommands.getWaypointCoordinates, index)
end
...
briefingRoom.f10Menu.missionMenu = missionCommands.addSubMenuForCoalition(briefingRoom.playerCoalition, "Миссия", nil)
missionCommands.addCommandForCoalition(briefingRoom.playerCoalition, "Статус миссии", briefingRoom.f10Menu.missionMenu, briefingRoom.f10MenuCommands.missionStatus, nil)

briefingRoom.f10Menu.objectiveMenu = missionCommands.addSubMenuForCoalition(briefingRoom.playerCoalition, "Цели", nil)
...
briefingRoom.mission.objectives[1] = { complete = false, failed = false, groupName = "27th tactical quarters-TGT-NAPPY", hideTargetCount = false, name = "NAPPY", targetCategory = Unit.Category.STRUCTURE, taskType = "DestroyAll", task = "Destroy all military structures designated objective NAPPY.", unitsCount = #dcsExtensions.getUnitNamesByGroupNameSuffix("-TGT-NAPPY"), unitNames = dcsExtensions.getUnitNamesByGroupNameSuffix("-TGT-NAPPY"), progressionHidden = false,progressionHiddenBrief = false,progressionCondition = "", startMinutes = 0,f10MenuText = "Цель NAPPY",f10Commands = {}}
briefingRoom.mission.objectives[1].waypoint = { ["x"] = -264769.71875, ["y"] = 815021.0625 }
...
function briefingRoom.mission.destroyCallout(objectiveIndex, killedUnit, eventID, playerName)
  ...
  local messages = { "Цель уничтожена.", "Хорошее попадание в цель.", "Цель разлетелась.", "Цель сбита!" }
  local targetType = "Ground"
  local messageIndex = math.random(1, 2)
  local messageIndexOffset = 0
  ...
  if briefingRoom.eventHandler.BDASetting == "ALL" or briefingRoom.eventHandler.BDASetting == "TARGETONLY" then
    briefingRoom.radioManager.play("Команда: "..(playerName or "").." "..messages[messageIndex + messageIndexOffset], "RadioHQ"..soundName..targetType..tostring(messageIndex), math.random(1, 3))
  end
    ...
    for __,u in ipairs(briefingRoom.mission.objectives[objectiveIndex].unitNames) do
      ...
      if unit ~= nil then
        ...
        if distance < briefingRoom.mission.objectiveDropDistanceMeters and not unit:inAir() then
          briefingRoom.radioManager.play("Пилот: Команда, Груз доставлен.", "RadioPilotCargoDelivered")
...
function briefingRoom.mission.objectivesTriggersCommon.registerKeepAliveTrigger(objectiveIndex)
  table.insert(briefingRoom.mission.objectiveTriggers, function(event)
    ...
    -- Play "target destroyed" radio message
    local messages = { "Команда: Наземные войска, которые вы должны защищать, несут потери!", "Команда: Мы потеряли дружественный отряд!" }
    local messageIndex = math.random(1, 2)
    local messageIndexOffset = 0
        ...
        if unit ~= nil then
          ...
          if distance < 3704 and math.abs(vec2p.y - vec2u.y) < 609.6 and p:inAir() then -- less than 2nm away on the X/Z axis, less than 2000 feet of altitude difference
            ...
            if math.random(1, 2) == 1 then
              briefingRoom.radioManager.play((playername or"Пилот")..": Команда, у меня хорошая видимость цели.", "RadioPilotTargetReconned1")
            else
              briefingRoom.radioManager.play((playername or"Пилот")..": Команда, положительный визуальный эффект на цель.", "RadioPilotTargetReconned2")
            end
            briefingRoom.mission.objectives[objectiveIndex].unitNames = { }
            briefingRoom.mission.coreFunctions.completeObjective(objectiveIndex)
            return nil
...
function briefingRoom.mission.objectivesTriggersCommon.escortNearTriggerlaunchMission (args)
  ...
  briefingRoom.radioManager.play("Пилот: Сопровождайте "..objective.name..", вы готовы начать.", "RadioPilotBeginEscort")
  ...
  if unit ~= nil then
    ...
    if group ~= nil then
      group:activate()
      briefingRoom.radioManager.play("Эскорт "..objective.name..": Сопровождайте "..objective.name..", вы готовы начать.", "RadioEscortMoving", briefingRoom.radioManager.getAnswerDelay(), nil, nil)
    end
  end
end
...
function briefingRoom.mission.objectivesTriggersCommon.registerEscortNearTrigger(objectiveIndex)
    ...
    for __,u in ipairs(briefingRoom.mission.objectives[objectiveIndex].unitNames) do
      ...
      if unit ~= nil then
        ...
        if distance < trigDistanceMeters then
          table.removeValue(briefingRoom.mission.objectives[objectiveIndex].unitNames, u)
          if table.count(briefingRoom.mission.objectives[objectiveIndex].unitNames) < 1 then -- all target units destroyed, objective complete
            briefingRoom.radioManager.play("Пилот: Командуйте, дружественные силы достигли своей цели.", "RadioPilotEscortComplete")
            briefingRoom.mission.coreFunctions.completeObjective(objectiveIndex)
          end
        end
      end
  ...
  if unit ~= nil and not unit:isActive() then
    table.insert(briefingRoom.mission.objectives[objectiveIndex].f10Commands, {text = "Запустить миссию", func = briefingRoom.mission.objectivesTriggersCommon.escortNearTriggerlaunchMission, args =  {objectiveIndex}})
  end
end

function briefingRoom.mission.objectivesTriggersCommon.flyNearAndReportComplete(args)
  local objectiveIndex = args[1]
  briefingRoom.radioManager.play("Пилот: Команда, цель отчета выполнена.", "RadioPilotReportComplete", math.random(1, 3))
  briefingRoom.mission.coreFunctions.completeObjective(objectiveIndex)
      ...
      for __,u in ipairs(briefingRoom.mission.objectives[objectiveIndex].unitNames) do
        ...
        if unit ~= nil then
          ...
          if distance < 9260 and math.abs(vec2p.y - vec2u.y) < 2438 and briefingRoom.mission.objectives[objectiveIndex].completeCommand == nil then
            ...
            briefingRoom.mission.objectives[objectiveIndex].unitNames = { }
            briefingRoom.mission.objectives[objectiveIndex].completeCommand = missionCommands.addCommandForCoalition(briefingRoom.playerCoalition, "Отчет завершен", briefingRoom.f10Menu.objectives[objectiveIndex],  briefingRoom.mission.objectivesTriggersCommon.flyNearAndReportComplete, {objectiveIndex})
          end
        end
      end
    end
...
function briefingRoom.mission.objectivesTriggersCommon.registerTransportTroopsTrigger(objectiveIndex)
  table.insert(briefingRoom.mission.objectives[objectiveIndex].f10Commands, {text = "Прикажите войскам погрузиться (все подходящие самолеты)", func = briefingRoom.mission.objectivesTriggersCommon.transportTroopsForcePickup, args = {objectiveIndex}})
  table.insert(briefingRoom.mission.objectives[objectiveIndex].f10Commands, {text = "Приказать войскам покинуть (все пригодные самолеты)", func =  briefingRoom.mission.objectivesTriggersCommon.transportTroopsForceDrop, args =  {objectiveIndex}})
    ...
    for __,u in ipairs(briefingRoom.mission.objectives[objectiveIndex].unitNames) do
      ...
      if unit ~= nil then
        ...
        if distance < briefingRoom.mission.objectiveDropDistanceMeters then
          table.removeValue(briefingRoom.mission.objectives[objectiveIndex].unitNames, u)
          if table.count(briefingRoom.mission.objectives[objectiveIndex].unitNames) < 1 then -- all target units destroyed, objective complete
            briefingRoom.radioManager.play("Пилот: Командование, Войска доставлены.", "RadioPilotTroopsDelivered")
            briefingRoom.mission.coreFunctions.completeObjective(objectiveIndex)
          end
        end
      end
  ...
  table.insert(briefingRoom.mission.objectiveTriggers, function(event)
    ...
    if distanceToObjective < briefingRoom.mission.objectiveDropDistanceMeters then
      ...
      if table.count(briefingRoom.mission.objectives[objectiveIndex].unitNames) < 1 then -- all target units moved or dead, objective complete
        ...
        briefingRoom.radioManager.play((playername or"Пилот")..": Командование, Войска доставлены.", "RadioPilotTroopsDelivered")
        briefingRoom.mission.coreFunctions.completeObjective(objectiveIndex)
      end
      return true
    end
...
briefingRoom.mission.objectivesTriggersCommon.registerDestroyTrigger(1)
    ...
    function table:onEvent(event)
        if event.id == world.event.S_EVENT_MARK_REMOVED then
            if dataObj.markID ~= nil and event.idx == dataObj.markID then
                if not dataObj.disableCooRemovedRadioMessage then
                    briefingRoom.radioManager.play(": Подтвердить, координаты удалены. Ждем новых координат.", "RadioCoordinatesDiscardedM")
                end
                dataObj.markID = nil
            end
        elseif event.id == world.event.S_EVENT_MARK_ADDED then
            ...
            if markText == dataObj.MARKER_NAME then
                ...
                briefingRoom.radioManager.play(dataObj.LANG_UNIT .. ": Копировать, координаты обновлены.",
        ...
        elseif event.id == world.event.S_EVENT_MARK_CHANGE then
            ...
            elseif dataObj.markID ~= nil and event.idx == dataObj.markID then
                briefingRoom.radioManager.play(dataObj.LANG_UNIT .. ": Подтвердить, координаты удалены. Ждем новых координат.",
...
function briefingRoom.taskables.launchCurry(dataObj, groupName, newTaskFunc)
    briefingRoom.radioManager.play("Пилот: " .. dataObj.LANG_REQEUEST, dataObj.RADIO_REQEUEST)
...
function briefingRoom.mission.objectiveFeaturesCommon.resgiterFireNearby(objectiveIndex)
  ...
  if unit == nil then -- no unit nor static found with the ID
    briefingRoom.radioManager.play(objective.name.." JTAC: Отрицательный, нет изображения ни на одной цели.", "RadioSupportNoTarget", briefingRoom.radioManager.getAnswerDelay())
    return
  end
...
function briefingRoom.mission.objectivesTriggersCommon.startAttack(args)
  local objectiveIndex = args[1]
  briefingRoom.radioManager.play("Пилот: Сухопутные войска, я готов поддержать, начать атаку", "RadioPilotBeginYourAttack")
  local groupNames = dcsExtensions.getGroupNamesContaining("%-STGT%-0")
  ...
  briefingRoom.radioManager.play("Отряд: Копировать, начинаю атаку.", "RadioOtherPilotBeginAttack")
...
table.insert(briefingRoom.mission.objectives[objectiveIndex].f10Commands, {text = "Начать атаку", func = briefingRoom.mission.objectivesTriggersCommon.startAttack, args = {objectiveIndex}})
...
function briefingRoom.mission.objectiveFeaturesCommon.targetDesignationCoordinates(args)
  local objectiveIndex = args[1]
  briefingRoom.radioManager.play("Пилот: Требовать обновления координат цели.", "RadioPilotTargetCoordinates")
  local objective = briefingRoom.mission.objectives[objectiveIndex]
  ...
  briefingRoom.radioManager.play(objective.name.." JTAC: Подтвердить, передавая обновленные координаты цели\n"..cooMessage, "RadioSupportTargetCoordinates", briefingRoom.radioManager.getAnswerDelay())

  local idx = briefingRoom.mission.objectiveFeatures[objectiveIndex].objRadioCommandIndex

  missionCommands.removeItemForCoalition(briefingRoom.playerCoalition, briefingRoom.mission.objectives[objectiveIndex].f10Commands[idx].commandPath)
  briefingRoom.mission.objectives[objectiveIndex].f10Commands[idx].commandPath = missionCommands.addCommandForCoalition(briefingRoom.playerCoalition, "Требовать целевые координаты.\nПоследние координаты:\n"..cooMessage, briefingRoom.f10Menu.objectives[objectiveIndex], briefingRoom.mission.objectiveFeatures[objectiveIndex].targetDesignationCoordinates)
end



function briefingRoom.mission.objectiveFeaturesCommon.registerTargetDesignationCoordinates(objectiveIndex)
  table.insert(briefingRoom.mission.objectives[objectiveIndex].f10Commands, {text = "Требовать целевые координаты", func = briefingRoom.mission.objectiveFeaturesCommon.targetDesignationCoordinates, args =  {objectiveIndex}})
...
function briefingRoom.mission.objectiveFeaturesCommon.targetDesignationFlare(args)
  local objectiveIndex = args[1]
  briefingRoom.radioManager.play("Пилот: У меня нет изображенияя на тебе. Умеешь ли ты стрелять сигнальной ракетой?", "RadioPilotMarkSelfWithFlare")
  local objective = briefingRoom.mission.objectives[objectiveIndex]
  ...
  if objectiveFeature.targetDesignationFlareFlaresLeft <= 0 then
    briefingRoom.radioManager.play(objective.name.." JTAC: Отрицательно, у меня закончились вспышки.", "RadioSupportShootingFlareOut", briefingRoom.radioManager.getAnswerDelay())
    return
  end
  ...
  if briefingRoom.mission.objectiveFeatures[objectiveIndex].flareInnacurate then 
    ...
    briefingRoom.radioManager.play(objective.name.." JTAC: Подтверждаем, сейчас стреляем сигнальной ракетой (оставлена сигнальная ракета: "..tostring(objectiveFeature.targetDesignationFlareFlaresLeft..") "..dcsExtensions.degreesToCardinalDirection(heading).." of the Flare.", "RadioSupportShootingFlare", briefingRoom.radioManager.getAnswerDelay(), briefingRoom.mission.objectiveFeaturesCommon.targetDesignationFlareDoFlare, args)
  else
    briefingRoom.radioManager.play(objective.name.." JTAC: Подтверждаем, сейчас стреляем сигнальной ракетой (оставлена сигнальная ракета: "..tostring(objectiveFeature.targetDesignationFlareFlaresLeft..")", "RadioSupportShootingFlare", briefingRoom.radioManager.getAnswerDelay(), briefingRoom.mission.objectiveFeaturesCommon.targetDesignationFlareDoFlare, args)
  end
end
...
function briefingRoom.mission.objectiveFeaturesCommon.registerTargetDesignationFlare(objectiveIndex, innacurate)
  ...
  table.insert(briefingRoom.mission.objectives[objectiveIndex].f10Commands, {text = "Запустить вспышку", func = briefingRoom.mission.objectiveFeaturesCommon.targetDesignationFlare, args =  {objectiveIndex}})
end
...
function briefingRoom.mission.objectiveFeaturesCommon.targetDesignationIlluminationBomb(args)
  local objectiveIndex = args[1]
  briefingRoom.radioManager.play("Пилот: Разведка, можете ли вы сбросить на цель осветительную бомбу?", "RadioPilotDropIlluminationBomb")
  local objective = briefingRoom.mission.objectives[objectiveIndex]
  local objectiveFeature = briefingRoom.mission.objectiveFeatures[objectiveIndex]
  -- out of bombs
  if objectiveFeature.targetDesignationIlluminationBombBombsLeft <= 0 then
    briefingRoom.radioManager.play(objective.name.." Разведка: Отрицательно, я Винчестер. Бомбы не осталось.", "RadioSupportIlluminationBombOut", briefingRoom.radioManager.getAnswerDelay())
    return
  end
  
  if table.count(briefingRoom.mission.objectives[objectiveIndex].unitNames) == 0 then -- no target units left
    briefingRoom.radioManager.play(objective.name.." Разведка:Отрицательный, нет изображения ни на одной цели.", "RadioSupportNoTarget", briefingRoom.radioManager.getAnswerDelay())
    return
  end
  ...
  if briefingRoom.mission.objectiveFeatures[objectiveIndex].illuminationBombInnacurate then 
    ...
    args.position.x = args.position.x - math.cos(heading * DEGREES_TO_RADIANS) * distance;
    args.position.z = args.position.z - math.sin(heading * DEGREES_TO_RADIANS) * distance;
    briefingRoom.radioManager.play(objective.name.." Разведка: Подтвердите, бомбите! (бомба(и) слева: "..tostring(objectiveFeature.targetDesignationIlluminationBombBombsLeft..") "..dcsExtensions.degreesToCardinalDirection(heading).." of the Illumination.", "RadioSupportIlluminationBomb", briefingRoom.radioManager.getAnswerDelay(), briefingRoom.mission.objectiveFeaturesCommon.targetDesignationIlluminationBombDoBomb, args)
  else
    briefingRoom.radioManager.play(objective.name.." Разведка: Подтвердите, бомбите! (бомба(и) слева: "..tostring(objectiveFeature.targetDesignationIlluminationBombBombsLeft..")", "RadioSupportIlluminationBomb", briefingRoom.radioManager.getAnswerDelay(), briefingRoom.mission.objectiveFeaturesCommon.targetDesignationIlluminationBombDoBomb, args)
  end
end
...
table.insert(briefingRoom.mission.objectives[objectiveIndex].f10Commands, {text = "Сбросить осветительную бомбу.", func = briefingRoom.mission.objectiveFeaturesCommon.targetDesignationIlluminationBomb, args =  {objectiveIndex}})
...
function briefingRoom.mission.objectiveFeaturesCommon.targetDesignationLaser.laserWatch(args, time)
  ...
  if not objFeature.targetDesignationLaser.laserTarget:isExist() or not table.contains(objective.unitNames, objFeature.targetDesignationLaser.laserTarget:getName()) then -- target is considered complete
    briefingRoom.debugPrint("JTAC objectiveIndex: Цель уничтожена, обнаружена новая цель", 1)
    ...
    if unit == nil then
      ...
      briefingRoom.radioManager.play(objective.name.." JTAC: Нет изображения ни на одной цели, лазер выключен.", "RadioSupportLasingNoMoreTargets", briefingRoom.radioManager.getAnswerDelay())
      return
    end
    briefingRoom.radioManager.play(objective.name.." JTAC: Рисуем следующую цель.", "RadioSupportLasingNextTarget", briefingRoom.radioManager.getAnswerDelay())
  end
...
function briefingRoom.mission.objectiveFeaturesCommon.targetDesignationLaser.turnOn(args)
  ...
  local objFeature = briefingRoom.mission.objectiveFeatures[objectiveIndex]
  local objective = briefingRoom.mission.objectives[objectiveIndex]
  briefingRoom.radioManager.play("Пилот: Можете ли вы нарисовать мне мишень?", "RadioPilotLaseTarget")

  -- already lasing something
  if objFeature.targetDesignationLaser.laserTarget ~= nil then
    briefingRoom.radioManager.play(objective.name.." JTAC: Цель уже закрашивается. Проверьте код лазера. Лазерный код "..tostring(objFeature.targetDesignationLaser.laserCode)..".", "RadioSupportTargetLasingAlready", briefingRoom.radioManager.getAnswerDelay())
    return
  end
  ...
  local unit = briefingRoom.mission.objectiveFeaturesCommon.targetDesignationLaser.setRandomTarget(objectiveIndex)
  if unit == nil then
    briefingRoom.radioManager.play(objective.name.." JTAC: Целей больше не видно. Лазер выключен.", "RadioSupportNoTarget", briefingRoom.radioManager.getAnswerDelay())
    return
  end
  briefingRoom.radioManager.play(objective.name.." JTAC: Подтвердить. Лазер включен, теперь рисуем цель. Лазерный код "..tostring(objFeature.targetDesignationLaser.laserCode)..".", "RadioSupportLasingOk", briefingRoom.radioManager.getAnswerDelay())
  missionCommands.addCommandForCoalition(briefingRoom.playerCoalition, "Лазерить другую цель", briefingRoom.f10Menu.objectives[objectiveIndex], briefingRoom.mission.objectiveFeaturesCommon.targetDesignationLaser.newTarget, {objectiveIndex})
  missionCommands.addCommandForCoalition(briefingRoom.playerCoalition, "Остановить лазерную мишень", briefingRoom.f10Menu.objectives[objectiveIndex], briefingRoom.mission.objectiveFeaturesCommon.targetDesignationLaser.turnOff, {objectiveIndex})
end
...
function briefingRoom.mission.objectiveFeaturesCommon.targetDesignationLaser.turnOff(args)
  ...
  local objFeature = briefingRoom.mission.objectiveFeatures[objectiveIndex]
  local objective = briefingRoom.mission.objectives[objectiveIndex]
  briefingRoom.radioManager.play("Пилот: Terminate. Laser off.", "RadioPilotLaseTargetStop")
  -- not lasing anything
  if objFeature.targetDesignationLaser.laserTarget == nil then
    briefingRoom.radioManager.play(objective.name.." JTAC: Невозможно выполнить, ничего не генерируется.", "RadioSupportLasingNotLasing", briefingRoom.radioManager.getAnswerDelay())
    return
  end

  briefingRoom.mission.objectiveFeaturesCommon.targetDesignationLaser.deleteLaser(objectiveIndex)
  briefingRoom.radioManager.play(objective.name.." JTAC: Копировать. Завершить, лазер выключен.", "RadioSupportLasingStopped", briefingRoom.radioManager.getAnswerDelay())
end
...
function briefingRoom.mission.objectiveFeaturesCommon.targetDesignationLaser.newTarget(args)
  local objectiveIndex = args[1]
  local objective = briefingRoom.mission.objectives[objectiveIndex]
  briefingRoom.radioManager.play("Пилот: Можете ли вы нарисовать для меня другую цель?", "RadioPilotLaseDiffrentTarget")
...
table.insert(briefingRoom.mission.objectives[objectiveIndex].f10Commands, {text = "Указать цель лазером", func = briefingRoom.mission.objectiveFeaturesCommon.targetDesignationLaser.turnOn, args =  {objectiveIndex}})
end
...
function briefingRoom.mission.objectiveFeaturesCommon.targetDesignationSmokeMarker(args)
  local objectiveIndex = args[1]
  local objective = briefingRoom.mission.objectives[objectiveIndex]
  briefingRoom.radioManager.play("Пилот: У меня нет визуала. Поставьте дымовую гранату в цель.", "RadioPilotMarkTargetWithSmoke")
  ...
  if timeNow < briefingRoom.mission.objectiveFeatures[objectiveIndex].targetDesignationSmokeMarkerNextSmoke then -- Smoke not ready
    briefingRoom.radioManager.play(objective.name.." JTAC: Цель уже отмечена дымом. Проверьте свою позицию, вы должны это увидеть.", "RadioSupportTargetAlreadyMarkedWithSmoke", briefingRoom.radioManager.getAnswerDelay())
    return
  end
  ...
  if briefingRoom.mission.objectiveFeatures[objectiveIndex].smokeInnacurate then 
    ...
    args.position.x = args.position.x - math.cos(heading * DEGREES_TO_RADIANS) * distance;
    args.position.z = args.position.z - math.sin(heading * DEGREES_TO_RADIANS) * distance;
    briefingRoom.radioManager.play(objective.name.." JTAC: Подтвердить, цель отмечена дымом. "..dcsExtensions.degreesToCardinalDirection(heading).." of the smoke.", "RadioSupportTargetMarkedWithSmoke", briefingRoom.radioManager.getAnswerDelay(), briefingRoom.mission.objectiveFeaturesCommon.targetDesignationSmokeMarkerDoSmoke, args)
  else
    briefingRoom.radioManager.play(objective.name.." JTAC: Подтвердить, цель отмечена дымом.", "RadioSupportTargetMarkedWithSmoke", briefingRoom.radioManager.getAnswerDelay(), briefingRoom.mission.objectiveFeaturesCommon.targetDesignationSmokeMarkerDoSmoke, args)
  end
end
...
function briefingRoom.mission.objectiveFeaturesCommon.registerTargetDesignationSmoke(objectiveIndex, innacurate)
  ...
  -- Add the command to the F10 menu
  table.insert(briefingRoom.mission.objectives[objectiveIndex].f10Commands, {text = "Отметить цель дымом", func = briefingRoom.mission.objectiveFeaturesCommon.targetDesignationSmokeMarker, args = {objectiveIndex}})
end
...
briefingRoom.mission.objectiveFeaturesCommon.registerTargetDesignationCoordinates(1)
briefingRoom.mission.objectiveFeaturesCommon.registerTargetDesignationLaser(1, 1688)
briefingRoom.mission.objectiveFeaturesCommon.registerTargetDesignationSmoke(1, false)
...
briefingRoom.mission.missionFeatures.groupNames.friendlyAWACS = "Focus 9-IQ-"
briefingRoom.mission.missionFeatures.unitNames.friendlyAWACS = {"Focus 9 1"}
briefingRoom.mission.missionFeatures.groupNames.friendlyTankerBasket = "Texaco 8-IQ-"
briefingRoom.mission.missionFeatures.unitNames.friendlyTankerBasket = {"Texaco 8 1"}
briefingRoom.mission.missionFeatures.groupNames.friendlyTankerBoom = "Arco 7-IQ-"
briefingRoom.mission.missionFeatures.unitNames.friendlyTankerBoom = {"Arco 7 1"}
briefingRoom.mission.missionFeatures.groupNames.friendlyStaticBuildings = "429th miscellaneous object"
briefingRoom.mission.missionFeatures.unitNames.friendlyStaticBuildings = {"429th miscellaneous object 1"}
briefingRoom.mission.missionFeatures.groupNames.friendlyTaskableArtillery = "725th Fire support brigade"
briefingRoom.mission.missionFeatures.unitNames.friendlyTaskableArtillery = {"725th Fire support brigade 1","725th Fire support brigade 2","725th Fire support brigade 3"}
...
briefingRoom.mission.missionFeatures.supportArtillery.LANG_UNIT = "Огневая поддержка"
...
function briefingRoom.mission.missionFeatures.supportArtillery.doShell(args, time)
  ...
  for i = 1, 3 do
    ...
    local inaccuracy = math.randomPointInCircle({ ["x"] = 0, ["y"] = 0 },
      briefingRoom.mission.missionFeatures.supportArtillery.INACCURACY)
    ...
    if i == 1 then
      ...
      for _, u in ipairs(enemyUnits) do
        if dcsExtensions.getDistance(dcsExtensions.toVec2(u:getPoint()), impactPointV2) <
            briefingRoom.mission.missionFeatures.supportArtillery.AUTO_AIM_RADIUS then
          if not u:inAir() then
...
function briefingRoom.mission.missionFeatures.supportArtillery.launchFireMission()
  briefingRoom.radioManager.play("Пилот: Огневая поддержка, начать огневую миссию по предоставленным координатам.", "RadioPilotArtillery")

  if briefingRoom.mission.missionFeatures.supportArtillery.fireMissionsLeft <= 0 then
    briefingRoom.radioManager.play("Огневая поддержка: Отрицательно, огневые миссии недоступны.", "RadioArtilleryNoAmmo",
      briefingRoom.radioManager.getAnswerDelay())
    return
  end
  ...
  for _, m in ipairs(marks) do
    if briefingRoom.mission.missionFeatures.supportArtillery.markID ~= nil and
      ...
      briefingRoom.mission.missionFeatures.supportArtillery.fireMissionsLeft = briefingRoom.mission.missionFeatures
          .supportArtillery
          .fireMissionsLeft - 1
      briefingRoom.radioManager.play("Огневая поддержка: Копировать, стрельба по предоставленным координатам ("..tostring(briefingRoom.mission.missionFeatures.supportArtillery.fireMissionsLeft).." Миссия(и) огня осталась).", "RadioArtilleryFiring",
  ...
  briefingRoom.radioManager.play("Огневая поддержка: Невозможно выполнить. Координаты огневой миссии не указаны (добавьте маркер с именем '"..string.upper(briefingRoom.mission.missionFeatures.supportArtillery.MARKER_NAME).."' на карте F10, чтобы обозначить цель).", "RadioArtilleryNoCoordinates",
    briefingRoom.radioManager.getAnswerDelay())
end
...
missionCommands.addCommandForCoalition(briefingRoom.playerCoalition, "Начать огневую миссию по предоставленным координатам", briefingRoom.f10Menu.missionMenu
...
for i=1,table.count(briefingRoom.mission.objectives) do
  table.insert(briefingRoom.mission.objectives[i].f10Commands, {text = "Запросить координаты путевой точки", func = briefingRoom.f10MenuCommands.getWaypointCoordinates, args =  i})
